...
<div class="w-full rounded-lg bg-slate-900 p-10 text-sm text-indigo-300 sm:w-96 max-w-96 mt-18">
    ...
    <form onSubmit=handleEvent(event)>
        <div class="mb-5 flex gap-3 rounded-full bg-[#333A5c] px-6 py-3">
            ...
            <input type="text" placeholder="Email" class="border-none outline-none" />
        </div>
        <div class="mb-5 flex gap-3 rounded-full bg-[#333A5c] px-6 py-3">
            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4">
                <path
            ...
            <input type="text" placeholder="Password" class="border-none outline-none" />
        </div>
        <p class="mb-6 mr-2 text-sm flex justify-end cursor-pointer"><a href="{{ route('forgot-password') }}"
                class="text-blue-400">Forgot
                Password?</a></p>
        <button
    ...
    <p class="mt-4 mb-7 text-center text-sm text-slate-400">Don't Have an account? <a href="{{ route('signup') }}"
    ...
    <div class="mt-6 flex justify-center w-full gap-6">
        <button
            class="bg-[#333A5c] px-4 py-2 w-full gap-0.5 max-w-56 rounded-lg flex justify-center items-center cursor-pointer">
