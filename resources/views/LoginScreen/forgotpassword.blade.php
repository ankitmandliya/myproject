...
<div class="w-full max-w-96 rounded-lg bg-slate-900 p-10 text-sm text-indigo-300 sm:w-96 mt-18">
    ...
    <form>
        <div class="mb-5 flex gap-3 rounded-full bg-[#333A5c] px-6 py-3">
            ...
            <input type="text" placeholder="Email" class="border-none outline-none" required />
        </div>
        <button type="button" onclick="window.location.href='{{ route('verification.code') }}'"
            class="w-full cursor-pointer rounded-full bg-gradient-to-r from-indigo-400 to-indigo-900 py-3 font-medium tracking-wide text-white">
            Reset Password
        </button>
    </form>
    <p class="mt-5 text-center text-sm">
        <a href="{{ route('login') }}" class="flex items-center justify-center gap-2 text-slate-400 hover:underline">
